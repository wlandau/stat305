...
\begin{frame}[fragile]
\frametitle{Example: cars}
<<fig=T,echo=F, out.width=".8\\textwidth", out.height=".8\\textheight">>=
curve(dexp, from =0, to=5, main="Density of X_1",xlab="x_1", ylab="Density of X_1")
@
\end{frame}
...
<<fig=T,echo=F, out.width=".8\\textwidth", out.height=".8\\textheight">>=
dens=function(x){
  return(dgamma(x,shape=44,scale=1/44))
}

dnorm2=function(x){
  return(dnorm(x,mean=44,sd=1/sqrt(44)))
}


curve(dens, from =1-4/sqrt(44), to=1+4/sqrt(44), main="Density of Average(X)",xlab="x", ylab="Density")
...
dnorm2=function(x){
  return(dnorm(x,mean=1,sd=1/sqrt(44)))
}


curve(dens, from =1-4/sqrt(44), to=1+4/sqrt(44), main="Densities of and Average(X) and N(1,1/44)",xlab="x", ylab="Density")

curve(dnorm2, from =1-4/sqrt(44), to=1+4/sqrt(44),main="Density of average(X)",xlab="x_1", ylab="Density of average(X)",col="blue", add=T)

legend("topright", col=c("black", "blue"), legend=c("average(X)", "N(1, 1/44"), lty=1)
