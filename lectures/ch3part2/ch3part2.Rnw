...
\begin{frame}[fragile]
<<echo=F, fig.width = 6.5, fig.height=4.75>>=
  par(cex=1.25)
  r = readRDS("../../data/rds/bullets.rds")
  r = r[r$grain == 200,]
  qqnorm(r$penetration, data=T, main="Normal QQ plot: 200-grain bullet penetration")
  qqline(r$penetration)
@
\end{frame}
...
\item Here, $s = \sqrt{3.2} = \Sexpr{sqrt(3.2)}$
\end{itemize}
\item Range
\begin{itemize}
...
<<echo=F>>=
  fib = c(0,1,1,2,3,5)
  fibstupid = fib
  fibstupid[6] = 817263489
@

\begin{tabular}{l|c|c}
Data & $x_i$ & $y_i$  \\ \hline
Mean & \Sexpr{mean(fib)} &\Sexpr{mean(fibstupid)}  \\ 
Median & 1.5 & 1.5 \\ 
Mode & 1 & 1\\ 
Sample Variance & \Sexpr{var(fib)} & \Sexpr{var(fibstupid)} \\ 
Sample Std. Dev. & \Sexpr{sd(fib)} &\Sexpr{sd(fibstupid)} \\
Range & \Sexpr{max(fib) - min(fib)} & \Sexpr{max(fibstupid) - min(fibstupid)} \\ 
IQR & 2& 2\\ 
\end{tabular}
\end{frame}
