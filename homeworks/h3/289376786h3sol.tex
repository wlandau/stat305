...
\begin{itemize}
\item Identify the sample. {\color{red} The 17 days during which the number of printer jams was recorded.}
\item Identify the population. {\color{red} All the days (past, present, and future) during which we \emph{could} record or \emph{could have} recorded the number of printer jams.}
\item Calculate the sample mean. {\color{red} $\ov{x} = 86.7059$}.
\item Calculate the median. {\color{red} Sort the values and pick the middle value. Doing that gives a median of 72. }
\item Calculate the mode. {\color{red} Mode = 89 because 89 appears most frequently.}
\item Calculate the sample variance. {\color{red} $s^2 = \frac{1}{17} [(43 - 86.7059)^2 + (100 - 86.7059)^2 + \cdots] =$   $1.1989\times 10^{4}$}.
\item Calcluate the sample standard deviation. {\color{red} $s = \sqrt{s^2}$ = 109.4936}
\item Calculate the range. {\color{red} Range = 500 - 21 = 479}. 
\end{itemize}
...
\begin{enumerate}[a. ]
\item Find the IQR. {\color{red} IQR = Q(0.75) - Q(0.25) = 0.930 - 0.870 = 0.06}
...
\setkeys{Gin}{width=1\textwidth} \includegraphics{../../fig/ch3s2p1b.png}


\item Make and discuss the appearance of a Q-Q (quantile-quantile) plot for comparing
the shapes of these two data sets.

\setkeys{Gin}{width=1\textwidth} \includegraphics{../../fig/ch3s2p1c.png}
...
{\color{red} Since the data in the above normal quantile plot follow a straight line, the yield values are symmetric and bell-shaped (normally distributed). In practice, you don’t usually get data better than this.}
...
{\color{red} The above plot is dishonest and misleading. The vertical axis starts from 79.75 thousand harnesses, hiding most of the 80,000 faulty harnesses corresponding to the right-hand bar.}
...
{\color{red} With 80,000 faulty harnesses out of 180,000 harnesses produced in Jan. 2012, a recall is necessary.}
...
\setkeys{Gin}{width=.75\textwidth} \includegraphics{../../fig/ch4s1p3absol.png}
\setkeys{Gin}{width=.75\textwidth} \includegraphics{../../fig/ch4s1p3csol1.png}
\setkeys{Gin}{width=.75\textwidth} \includegraphics{../../fig/ch4s1p3csol2.png}


\item Vardeman and Jobe chapter 4 section 1 problem 4 parts a-c(page 140). Data can be found at \url{http://www.will-landau.com/stat305/data/csv/tools.csv}. Please do this problem by hand.

\setkeys{Gin}{width=.75\textwidth} \includegraphics{../../fig/ch4s1p4.png}

\setkeys{Gin}{width=1\textwidth} \includegraphics{../../fig/ch4s1p4asol.png}
\setkeys{Gin}{width=1\textwidth} \includegraphics{../../fig/ch4s1p4bsol.png}
\setkeys{Gin}{width=1\textwidth} \includegraphics{../../fig/ch4s1p4csol.png}
...
{\color{red} Here is the histogram of the original data. The distributional shape is approximately uniform.}


\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\includegraphics[width=\textwidth]{figure/unnamed-chunk-5} 
\end{knitrout}


{\color{red} However, there are shapes besides uniform that are also correct, such as...}

\setkeys{Gin}{width=1\textwidth} \includegraphics{../../fig/hw3extra.png}
